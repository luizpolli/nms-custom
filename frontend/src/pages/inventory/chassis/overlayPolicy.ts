...
export const OVERLAY_POLICY: OverlayPolicy = {
  // ASR920 base SVG draws the SFP cage outlines for all 20 bays — rendering
  // SFP.svg on top would double-draw the transceiver face.  Uplink and PSU
  // overlays are required: the base SVG shows the empty bay frame only, not
  // the module face artwork.
  Cisco_ASR_920_20SZ_M_Router: {
    sfp: false,
  },
  // ASR920 12-port variant profiles: the synthetic base SVG generated by
  // scripts/build_asr920_variant_profiles.py already draws every RJ45/SFP
  // cage, the uplinks, the IM slot and the PSUs at hotspot coordinates, so
  // module overlays would double-draw connectors (same as the 20SZ-M above).
  Cisco_ASR_920_12CZ_Router: {
    rj45: false,
    sfp: false,
    uplink: false,
    psu: false,
  },
  Cisco_ASR_920_12SZ_Router: {
    sfp: false,
    psu: false,
  },
  Cisco_ASR_920_12SZ_IM_Router: {
    rj45: false,
    sfp: false,
    uplink: false,
    psu: false,

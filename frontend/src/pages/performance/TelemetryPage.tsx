  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <Card className="p-4">
          <h2 className="mb-3 text-sm font-semibold text-gray-900 dark:text-white">New collector</h2>
        ...
        <Card className="p-4">
          <h2 className="mb-3 text-sm font-semibold text-gray-900 dark:text-white">New subscription</h2>
          <div className="space-y-3">
        ...
        <Card className="p-4">
          <h2 className="mb-3 text-sm font-semibold text-gray-900 dark:text-white">Sensor path mapping</h2>
          <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <TelemetryTable title="Collectors" empty="No telemetry collectors yet" rows={collectors.map((c) => [c.name, c.collector_type, c.endpoint || '—', <Badge key={c.id} variant={c.enabled ? 'success' : 'default'}>{c.status || (c.enabled ? 'enabled' : 'disabled')}</Badge>])} />
        <TelemetryTable title="Subscriptions" empty="No telemetry subscriptions yet" rows={subscriptions.map((s) => [s.name, s.path, `${s.sample_interval_ms}ms`, <Badge key={s.id} variant={s.enabled ? 'success' : 'default'}>{s.status}</Badge>])} />
      </div>

      <Card className="p-4">
        <div className="mb-3 flex items-center gap-2">
          <DatabaseZap className="h-5 w-5 text-cisco-blue" />
          <h2 className="text-sm font-semibold text-gray-900 dark:text-white">Sensor path catalog</h2>
        </div>
      ...
      <Card className="p-4">
        <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300"><Send className="h-4 w-4" /> Samples can be posted to <code className="rounded bg-gray-100 px-1 dark:bg-gray-800">POST /api/telemetry/samples</code>; normalized KPI rows and Redis Stream events are created by the backend.</div>
      </Card>
    </div>
  );
}

function TelemetryTable({ title, empty, rows }: { title: string; empty: string; rows: Array<Array<React.ReactNode>> }) {
  return <Card className="p-4"><h2 className="mb-3 text-sm font-semibold text-gray-900 dark:text-white">{title}</h2>{rows.length === 0 ? <EmptyState title={empty} /> : <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700"><table className="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700"><tbody className="divide-y divide-gray-100 dark:divide-gray-800">{rows.map((row, idx) => <tr key={idx}>{row.map((cell, i) => <Td key={i}>{cell}</Td>)}</tr>)}</tbody></table></div>}</Card>;
}
